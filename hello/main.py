...
def update(state, session):
    ...
    paginated_members = _get_paginated_members(
        state["paginated_members_page"] - 1, state["paginated_members_page_size"]
    )
...
def handle_paginated_members_page_change(state, payload):
    ...
    maxpage = (
        int(
            state["paginated_members_total_items"]
            / state["paginated_members_page_size"]
        )
        + 1
    )
    if page > maxpage:
...
def _update_role_chart(state):
    ...
    fig.update_layout(margin={"l": 20, "r": 20, "t": 20, "b": 50}, showlegend=False)
...
def on_editable_df_record_action(state, payload):
    ...
    if payload["action"] == "open":
        state["editable_df_open_text"] = str(
            state["editable_df"].record(record_index)["text"]
        )
    if payload["action"] == "notify":
        state.add_notification(
            "info",
            "Test",
            "Notify on this row : " + str(state["editable_df"].record(record_index)),
        )
